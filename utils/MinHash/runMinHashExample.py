...
if __name__ == '__main__':
    ...
    numDocs = 1000
    dataFile = dir + "/reference_code/MinHash/data/articles_" + str(numDocs) + ".train"
    truthFile = dir + "/reference_code/MinHash/data/articles_" + str(numDocs) + ".truth"
    ...
    for i in range(0, numDocs):
        ...
        for index in range(0, len(words) - 2):
            # Construct the shingle text by combining three words together.  3-gram
            shingle = words[index] + " " + words[index + 1] + " " + words[index + 2]
            # Hash the shingle to a 32-bit integer.
            crc = binascii.crc32(str.encode(shingle)) & 0xffffffff  # 3475589306 ->
    ...
    print('Shingling ' + str(numDocs) + ' docs took %.2f sec.' % (time.time() - t0))
    print('Average shingles per doc: %.2f' % (totalShingles / numDocs))
    # Shingling 1000 docs took 0.22 sec.
    ...
    cal_jaccard = False
    if numDocs <= 2500 and cal_jaccard:
    ...
    # Calculate the elapsed time (in seconds)
    elapsed = (time.time() - t0)
    print("Generating MinHash signatures took %.2fsec" % elapsed)
